...
class MenuBuilder:
    ...
    def _get_dynamic_menu_items(self) -> List:
        ...
        menu_items.append(
            pystray.MenuItem(copy_label, self._copy_datetime_wrapper, default=True)
        )

        search_hk = self._format_hotkey_display(self.hotkey_search_history)
        search_label = (
            f"Buscar no histórico ({search_hk})" if search_hk else "Buscar no histórico"
        )
        menu_items.append(pystray.MenuItem(search_label, self._show_search_wrapper))

        menu_items.append(
            pystray.MenuItem("Configurações…", self._show_custom_shortcuts_wrapper)
        )

        refresh_hk = self._format_hotkey_display(self.hotkey_refresh_menu)
        refresh_label = (
            f"Recarregar itens ({refresh_hk})" if refresh_hk else "Recarregar itens"
        )
                ...
                for idx, entry in enumerate(reversed(recent), start=1):
                    ...
                    def make_copy_func(txt):
                        return lambda icon, item: (
                            ...
                            if self.copy_from_history_callback
                            else None
                        )

                    copy_func = make_copy_func(text)
                    menu_items.append(
                        pystray.MenuItem(f"{idx}. {display_text}", copy_func)
                    )
            else:
                menu_items.append(
                    pystray.MenuItem("(Histórico vazio)", None, enabled=False)
                )
        ...
        menu_items.append(
            pystray.MenuItem("Limpar histórico", self._clear_history_wrapper)
        )
    ...
    def create_dynamic_menu(self) -> pystray.Menu:
        ...
        def dynamic_items():
            ...
            try:
                ...
                try:
                    cache_window_s = max(
                        0.0, float(self.tray_menu_cache_window_ms) / 1000.0
                    )
                except Exception:
                    cache_window_s = 0.2
