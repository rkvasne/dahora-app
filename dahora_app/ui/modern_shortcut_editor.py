...
class ModernShortcutEditor:
    ...
    def __init__(
        ...
        self.var_hotkey = ctk.StringVar(
            master=self.parent, value=self.shortcut.get("hotkey", "")
        )
        self.var_prefix = ctk.StringVar(
            master=self.parent, value=self.shortcut.get("prefix", "")
        )
        self.var_description = ctk.StringVar(
            master=self.parent, value=self.shortcut.get("description", "")
        )
        self.var_enabled = ctk.BooleanVar(
            master=self.parent, value=self.shortcut.get("enabled", True)
        )
    ...
    def _create_window(self) -> None:
        ...
        # Título
        ModernLabel(
            inner, text=f"{'➕' if is_new else '✏️'} {title}", style="title"
        ).pack(anchor="w", pady=(0, 14))

        # === PREFIXO ===
        ModernLabel(inner, text="Prefixo", style="heading").pack(
            anchor="w", pady=(0, 4)
        )
        ModernLabel(
            inner, text="Texto inserido antes da data/hora", style="muted"
        ...
        # === DESCRIÇÃO ===
        ModernLabel(inner, text="Descrição", style="heading").pack(
            anchor="w", pady=(0, 4)
        )
        ModernLabel(inner, text="Identificação opcional do atalho", style="muted").pack(
            anchor="w", pady=(0, 8)
        )

        ModernEntry(inner, textvariable=self.var_description, width=400).pack(
            anchor="w", pady=(0, 12)
        )

        # === ATALHO ===
        ModernLabel(inner, text="Combinação de Teclas", style="heading").pack(
            anchor="w", pady=(0, 4)
        )
        ModernLabel(
            inner, text="Ex: ctrl+shift+1, alt+f1, ctrl+alt+d", style="muted"
        ).pack(anchor="w", pady=(0, 8))

        hotkey_frame = ctk.CTkFrame(inner, fg_color="transparent")
        hotkey_frame.pack(fill="x", pady=(0, 8))

        hotkey_entry = ModernEntry(
            hotkey_frame, textvariable=self.var_hotkey, width=300
        )
        hotkey_entry.pack(side="left", padx=(0, 12))
        hotkey_entry.bind("<KeyRelease>", lambda e: self._validate_hotkey())

        ModernButton(
            hotkey_frame, text="🎹 Detectar", command=self._start_detecting
        ).pack(side="left")
        ...
        ModernLabel(preview_inner, text="Preview do Resultado", style="heading").pack(
            anchor="w", pady=(0, 8)
        )
    ...
    def _validate_hotkey(self) -> None:
        ...
        if not hotkey:
            self.validation_label.configure(
                text="Digite um atalho", text_color=self.colors["text_muted"]
            )
            return
        ...
        if self.on_validate_hotkey:
            ...
            if valid:
                self.validation_label.configure(
                    text="✓ Atalho válido", text_color=self.colors["success"]
                )
            else:
                self.validation_label.configure(
                    text=f"✗ {msg}", text_color=self.colors["error"]
                )
        else:
            self.validation_label.configure(
                text="", text_color=self.colors["text_muted"]
            )
    ...
    def _start_detecting(self) -> None:
        ...
        ModernLabel(
            container, text="Pressione a combinação de teclas...", style="heading"
        ).pack(pady=(0, 16))
        ...
        ModernButton(
            container,
            text="Cancelar",
            command=lambda: self._stop_detecting(detect, None),
        ).pack()
        ...
        def finalize_if_ready() -> None:
            mods = [
                m for m in ["ctrl", "shift", "alt"] if m in self._detect_pressed_mods
            ]
        ...
        y = (
            ...
            - (detect.winfo_height() // 2)
        )
        detect.geometry(f"+{x}+{y}")

    def _stop_detecting(
        self, detect_window: ctk.CTkToplevel, hotkey: Optional[str]
    ) -> None:
        """Para a detecção"""
        self.is_detecting = False
