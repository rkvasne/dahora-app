    ...
    def show(self) -> None:
        ...
        if not TKINTER_AVAILABLE:
            ...
            if self.notification_callback:
                self.notification_callback("Dahora App", "Tkinter não disponível. Não é possível abrir busca.")
            return
    ...
    def _show_dialog_thread(self) -> None:
        ...
        try:
            ...
            def perform_search(*args):
                ...
                # Atualiza contagem
                count_label.config(text=f"{len(filtered_results)} resultados encontrados")
            
            # Botão buscar
            search_button = ttk.Button(search_frame, text="Buscar", command=perform_search)
