...
class SettingsDialog:
    ...
    def show(self) -> None:
        ...
        logging.info(
            "show_settings_dialog() chamada - Iniciando janela de configurações"
        )
        ...
        if not TKINTER_AVAILABLE:
            ...
            if self.notification_callback:
                self.notification_callback(
                    "Dahora App",
                    "Tkinter não disponível. Não é possível abrir configurações.",
                )
            return
    ...
    def _show_dialog_thread(self) -> None:
        ...
        try:
            ...
            Windows11Style.configure_window(
                root, "Dahora App - Configurações", "480x350"
            )
            try:
            ...
            # Cabeçalho
            ttk.Label(
                main, text="Configurações Avançadas", font=(default_font, 12, "bold")
            ...
            ttk.Entry(frame_prefix, textvariable=var_prefix, width=40).pack(
                anchor=tk.W, pady=(2, 0)
            )
            ttk.Label(
                frame_prefix,
                text="Ex: [trabalho] (opcional)",
                font=(default_font, 8),
                foreground="gray",
                style="Card.TLabel",
            ).pack(anchor=tk.W)
            ...
            ttk.Label(
                frame_format,
                text="Formato de Data/Hora:",
                font=(default_font, 9),
                style="Card.TLabel",
            ).pack(anchor=tk.W)
            var_datetime_format = tk.StringVar(
                value=self.current_settings.get("datetime_format", "%d.%m.%Y-%H:%M")
            )
            ttk.Entry(frame_format, textvariable=var_datetime_format, width=40).pack(
                anchor=tk.W, pady=(2, 0)
            )
            ttk.Label(
                frame_format,
                text="Exemplo: %d.%m.%Y-%H:%M",
                font=(default_font, 8),
                foreground="gray",
                style="Card.TLabel",
            ).pack(anchor=tk.W)
            ...
            var_max_history = tk.IntVar(
                value=self.current_settings.get("max_history_items", 100)
            )
            ttk.Spinbox(
                frame_max, from_=10, to=1000, textvariable=var_max_history, width=10
            ).pack(side=tk.RIGHT)
            ...
            ttk.Label(
                frame_interval,
                text="Intervalo de verificação (s):",
                font=(default_font, 9),
                style="Card.TLabel",
            ).pack(side=tk.LEFT)
            var_monitor_interval = tk.DoubleVar(
                value=self.current_settings.get("clipboard_monitor_interval", 3)
            )
            ttk.Spinbox(
                frame_interval,
                from_=0.5,
                to=60,
                increment=0.5,
                textvariable=var_monitor_interval,
                width=10,
            ).pack(side=tk.RIGHT)
            ...
            ttk.Label(
                frame_idle,
                text="Tempo sem mudanças na área de transferência (s):",
                font=(default_font, 9),
                style="Card.TLabel",
            ).pack(side=tk.LEFT)
            var_idle_threshold = tk.DoubleVar(
                value=self.current_settings.get("clipboard_idle_threshold", 30)
            )
            ttk.Spinbox(
                frame_idle,
                from_=5,
                to=300,
                increment=5,
                textvariable=var_idle_threshold,
                width=10,
            ).pack(side=tk.RIGHT)
            ...
            ttk.Label(
                frame_duration,
                text="Duração da notificação (segundos):",
                font=(default_font, 9),
                style="Card.TLabel",
            ).pack(side=tk.LEFT)
            var_notification_duration = tk.IntVar(
            ...
            ttk.Spinbox(
                ...
                from_=1,
                to=10,
                textvariable=var_notification_duration,
                width=10,
            ).pack(side=tk.RIGHT)
            ...
            ttk.Label(
                frame_copy,
                text="Copiar Data/Hora:",
                font=(default_font, 9),
                style="Card.TLabel",
            ).pack(anchor=tk.W)
            var_hotkey_copy = tk.StringVar(
                value=self.current_settings.get("hotkey_copy_datetime", "ctrl+shift+q")
            )
            ttk.Entry(frame_copy, textvariable=var_hotkey_copy, width=30).pack(
                anchor=tk.W, pady=(2, 0)
            )
            ttk.Label(
                frame_copy,
                text="Ex: ctrl+shift+q",
                font=(default_font, 8),
                foreground="gray",
                style="Card.TLabel",
            ).pack(anchor=tk.W)
            ...
            ttk.Label(
                tab_hotkeys,
                text="✅ Alterações em atalhos são aplicadas automaticamente",
                font=(default_font, 8),
                foreground="gray",
                style="Card.TLabel",
            ...
            ttk.Label(
                tab_hotkeys,
                text="Configure múltiplos atalhos com prefixos diferentes",
                font=(default_font, 8),
                foreground="gray",
                style="Card.TLabel",
            ...
            ttk.Button(
                tab_hotkeys,
                text="⚙️ Gerenciar Atalhos Personalizados",
                command=on_manage_shortcuts,
            ...
            ttk.Button(buttons, text="Restaurar Padrões", command=on_reset).pack(
                side=tk.LEFT
            )
