...
class ShortcutEditorDialog:
    ...
    def __init__(
        ...
        on_save: Callable,
        on_validate_hotkey: Optional[Callable],
    ):
        """Inicializa o editor"""
        self.parent = parent
        ...
        try:
            ...
            description = (
                simpledialog.askstring(
                    ...
                    initialvalue=(
                        self.shortcut.get("description", "") if self.shortcut else ""
                    ),
                )
                or ""
            )
    ...
    def _create_window_content(self) -> None:
        ...
        try:
            ...
            ttk.Label(prefix_section, text="Prefixo", style="CardHeading.TLabel").pack(
                anchor="w", pady=(0, 6)
            )
            ttk.Label(
                prefix_section,
                text="Texto que será inserido antes da data/hora",
                style="Muted.TLabel",
            ).pack(anchor="w", pady=(0, 8))

            prefix_entry = Windows11Style.create_modern_entry(
                prefix_section, textvariable=self.prefix_var, width=40
            )
            ...
            ttk.Label(desc_section, text="Descrição", style="CardHeading.TLabel").pack(
                anchor="w", pady=(0, 6)
            )
            ttk.Label(
                desc_section,
                text="Descrição opcional para identificar este atalho",
                style="Muted.TLabel",
            ).pack(anchor="w", pady=(0, 8))

            desc_entry = Windows11Style.create_modern_entry(
                desc_section, textvariable=self.description_var, width=40
            )
            ...
            ttk.Label(
                hotkey_section, text="Combinação de Teclas", style="CardHeading.TLabel"
            ).pack(anchor="w", pady=(0, 6))
            ttk.Label(
                hotkey_section,
                text="Ex: ctrl+shift+1, alt+f1, ctrl+alt+d",
                style="Muted.TLabel",
            ).pack(anchor="w", pady=(0, 8))
            ...
            hotkey_entry = Windows11Style.create_modern_entry(
            ...
            detect_button = Windows11Style.create_modern_button(
                hotkey_frame, "Detectar", command=self._start_detecting
            )
            detect_button.pack(side=tk.RIGHT)

            # Validação
            self.validation_label = ttk.Label(
                hotkey_section, text="", style="Muted.TLabel"
            )
            ...
            ttk.Label(
                preview_card, text="Preview do Resultado", style="CardHeading.TLabel"
            ).pack(anchor="w", pady=(0, 8))
            ...
            cancel_btn = Windows11Style.create_modern_button(
                buttons_frame, "Cancelar", command=self._on_cancel_clicked
            )
            cancel_btn.pack(side=tk.RIGHT, padx=(12, 0))

            save_btn = Windows11Style.create_modern_button(
                buttons_frame,
                "Salvar",
    ...
    def _start_detecting(self) -> None:
        ...
        try:
            ...
            messagebox.showerror(
                "Erro",
                "Biblioteca 'keyboard' não disponível.\nInstale com: pip install keyboard",
            )
            return
        ...
        ttk.Button(
            detect_window,
            text="Cancelar",
            command=lambda: self._stop_detecting(detect_window, None),
        ).pack(pady=20)
        ...
        def on_key(event):
            ...
            if key_name in ["ctrl", "shift", "alt"]:
                ...
                # Monta hotkey
                modifiers = sorted(
                    [k for k in self.detected_keys if k in ["ctrl", "shift", "alt"]]
                )
                keys = [
                    k for k in self.detected_keys if k not in ["ctrl", "shift", "alt"]
                ]
            ...
            # Atualiza label
            keys_label.config(
                text=format_hotkey_display("+".join(sorted(self.detected_keys)))
            )
        ...
        x = (
            window.winfo_x()
            + (window.winfo_width() // 2)
            - (detect_window.winfo_width() // 2)
        )
        y = (
            window.winfo_y()
            + (window.winfo_height() // 2)
            - (detect_window.winfo_height() // 2)
        )
        detect_window.geometry(f"+{x}+{y}")

    def _stop_detecting(
        self, detect_window: tk.Toplevel, hotkey: Optional[str]
    ) -> None:
